...
\dfn{polyèdre, polytope}{
	On appelle \emphindex{polyèdre} l'intersection de demi-plans.
	On dit que le polyèdre est \emphindex{borné} si les coordonnées de ses points sont bornées en valeur absolue par un certain nombre (possiblement très grand).
	Un polyèdre borné est appelé un \emphindex{polytope}.
}{dfn:polyhedre}
...
\dfn{programme linéaire}{
	On appelle \emphindex{programme linéaire} un problème d'optimisation de fonction de la forme
		\begin{align}
		\text{minimiser }  \qquad \qquad & f(x_1, \dots, x_n) \label{expr:obj}  \\[10pt]
		\text{soumis à } \qquad \qquad & g_1(x_1, \dots, x_n) \leq b_1, \label{eq:constr1} \\[10pt]
									& \hspace{50pt}\vdots \nonumber \\[10pt]
									& g_m(x_1, \dots, x_n) \leq b_m, \label{eq:constrm}
		\end{align}
	où $f$ est une fonction linéaire appelée \emphindex{fonction coût}, et les $g_i$ sont des fonctions linéaires appelées \emphindex{contraintes linéaires}.
}{dfn:programme-linéaire}
